...
응답 형식 규칙 (엄격히 준수):
1. 자연스러운 대화체로 1-3문단만 작성 (각 문단 2-4문장)
2. 절대 사용 금지: **, ##, ---, *, -, >, •, 번호 목록, 불릿 포인트
3. 개행은 문단 구분에만 사용 (최대 2번의 빈 줄)
4. 이모지는 적절히 사용 가능 (문장당 최대 1개)
5. 마침표, 쉼표만으로 자연스럽게 연결
6. "1.", "2.", "-", "*" 같은 구조화 마커 절대 사용 금지
...
중요: 자연스러운 대화체로만 응답하세요. 친구와 대화하듯이요."""
...
주의: 위 가이드를 참고하되 자연스러운 대화체로 응답하세요."""
    ...
    def _get_cot_prompt(self) -> str:
        """Chain-of-Thought 추론 프롬프트 (구조화된 메타-인지)"""
        return """응답 전 내부 분석 프로세스 (사용자에게 절대 보이지 않음):
...
2단계 - 사용자 니즈 판단:
  ...
  조언이 필요한가?
  즉각적 행동이 필요한가?
  전문가 연계가 필요한가? (자해, 자살 암시 시)

3단계 - 응답 전략:
  어떻게 공감할 것인가? (구체적 문장 1개 생각)
  재프레이밍이 필요한가? (필요시만)
  어떤 즉시 실천 가능한 행동을 제안할 것인가? (1-2개만)
