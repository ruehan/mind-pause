...
주의: 위 가이드를 참고하되 자연스러운 대화체로 응답하세요."""
    ...
    def _get_cot_prompt(self) -> str:
        """Chain-of-Thought 추론 프롬프트 (메타-인지 형태)"""
        return """응답 전에 마음속으로 생각하세요:

먼저 사용자의 핵심 감정과 그 감정이 생긴 상황을 파악하세요. 사용자가 공감을 원하는지, 조언을 원하는지, 해결책을 원하는지 느껴보세요. 감정을 어떻게 인정하고 공감할지, 지금 당장 도움이 될 작은 행동이 무엇일지 생각하세요. 위기 상황이라면 전문가 도움을 권유할지 판단하세요.

이런 사고 과정은 마음속에만 담아두고, 사용자에게는 자연스럽고 따뜻한 한 문단의 대화로 응답하세요."""
