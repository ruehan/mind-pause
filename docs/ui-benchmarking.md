...
- [ ] 히어로 섹션: 큰 헤드라인 + 부드러운 페이드인 애니메이션
- [ ] 서비스 특징: 3-4개 카드로 간결하게 (아이콘 + 제목 + 한 줄 설명)
- [ ] 사용자 후기: 캐러셀 또는 그리드 레이아웃
- [ ] 통계 섹션: "000명이 마음쉼표와 함께 합니다" (신뢰감)
- [ ] CTA: "무료로 시작하기" 버튼 강조
...
- [ ] 감정 트렌드 차트: 라인 차트 or 바 차트 (7일/30일/90일 전환)
- [ ] 챌린지 진행률: 원형 프로그레스 또는 프로그레스 바
- [ ] 최근 AI 대화: 요약 카드 3개 표시
- [ ] 스트릭 표시: "연속 7일 기록 중!" 배지
- [ ] 알림/할 일 위젯: 오늘의 챌린지, 읽지 않은 알림
...
- [ ] 메시지 버블: 사용자(우측, 파랑) vs AI(좌측, 회색) 명확히 구분
- [ ] AI 아바타: react-nice-avatar로 커스터마이징한 아바타 표시
- [ ] 타이핑 인디케이터: "AI 코치가 입력 중..." 애니메이션
- [ ] 대화 히스토리: 좌측 사이드바에 이전 대화 목록 (제목 + 날짜)
- [ ] 음성 입력: 마이크 아이콘 버튼 (선택 기능)
- [ ] 감정 퀵 선택: "오늘 기분이 어떠세요?" 버튼 그룹
- [ ] 제안 질문 카드: "이런 이야기를 해보시겠어요?"
...
- [ ] 감정 스케일: -5 ~ +5 슬라이더 유지 (직관적)
- [ ] 감정 아이콘: 각 레벨별 이모지 표시 (😢😕😐🙂😊)
- [ ] 프롬프트: "오늘 어떤 일이 있었나요?" 질문 제시
- [ ] 메모 입력: 간단한 텍스트 에리어 (선택 사항)
- [ ] AI 피드백: 입력 후 즉시 긍정적 피드백 표시
- [ ] 주간 캘린더: 7일간 감정 아이콘 미니 뷰
- [ ] 월간 감정 차트: 라인 차트로 감정 변화 추이 표시
...
- [ ] 챌린지 카드: 이미지 썸네일 + 제목 + 간단한 설명
- [ ] 진행 중 챌린지: 큰 프로그레스 바 + 일일 완료 체크박스
- [ ] 스트릭 표시: "연속 14일 완료!" 불꽃 아이콘 🔥
- [ ] 완료 체크: 오늘 완료하면 체크 애니메이션
- [ ] 배지 시스템: "7일 연속 달성", "첫 챌린지 완료" 배지
- [ ] 카테고리 필터: 운동, 명상, 독서, 사회활동 등
...
- [ ] 게시글 카드: 제목 + 본문 미리보기 (2줄) + 공감/댓글 수
- [ ] 익명 아이콘: Lock 아이콘으로 익명 게시글 표시
- [ ] 공감 버튼: 하트 또는 손 아이콘 + 숫자
- [ ] 댓글 인라인: 게시글 카드 하단에 최근 댓글 1-2개 표시
- [ ] 감정 태그: #불안 #우울 #행복 #스트레스 등 해시태그
- [ ] 정렬/필터: 최신순, 인기순, 공감많은순, 댓글많은순
- [ ] 안전 가이드라인: 상단에 "서로 존중하는 커뮤니티" 배너
...
- [ ] 배지/성취: 그리드 레이아웃으로 획득한 배지 표시
- [ ] 프로필 완성도: "80% 완성되었습니다" 프로그레스 바
- [ ] 설정 탭: 계정, 알림, 보안, 개인정보, 테마, 언어
...
- [ ] 소셜 로그인 버튼: Google, Kakao, Naver (브랜드 컬러 적용)
- [ ] 익명 시작: "익명으로 시작하기" 버튼 (회색, 보조)
- [ ] 이메일 로그인: 구분선 "또는 이메일로" 추가
- [ ] 비밀번호 찾기: 링크 스타일로 하단에 배치
- [ ] 회원가입: 약관 동의 체크박스 추가
- [ ] 에러 메시지: 인라인으로 표시 (빨간색 텍스트)
...
- [ ] 카테고리:
  ...
  - 문제 해결
  - 기타
- [ ] FAQ 아코디언: 질문 클릭 시 답변 펼쳐짐
- [ ] 관련 글: 각 FAQ 하단에 "이 글도 도움이 될 수 있어요"
- [ ] 문의 옵션: 이메일, 채팅, 전화 (카드 형태)
- [ ] 비디오 튜토리얼: YouTube 임베드 (선택)
...
- [ ] 일러스트: 각 에러 타입별 맞춤 일러스트 or 아이콘
- [ ] 친근한 문구:
  - 404: "길을 잃으셨나요? 괜찮아요, 함께 찾아봐요!"
  - 500: "앗, 서버가 잠시 쉬고 있어요. 곧 돌아올게요!"
  - 403: "이 페이지는 특별 초대장이 필요해요."
- [ ] 해결 방법: 각 에러별 "이렇게 해보세요" 제안
- [ ] 액션 버튼: 홈으로, 뒤로가기, 새로고침
- [ ] 도움말 링크: "여전히 문제가 있나요? 도움말 보기"
