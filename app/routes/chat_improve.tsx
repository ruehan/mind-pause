...
import type { Route } from "./+types/chat_improve";
...
export default function ChatImprove() {
  ...
  useEffect(() => {
    const loadConversations = async () => {
      ...
      try {
        ...
        // Filter conversations by active character
        const filteredConvs = convs.filter(c => c.character_id === activeCharacter.id);
        setConversations(filteredConvs);
        
        // If no active conversation or active conversation not in filtered list, select first one
        if (filteredConvs.length > 0) {
          if (!activeConversationId || !filteredConvs.find(c => c.id === activeConversationId)) {
            setActiveConversationId(filteredConvs[0].id);
          }
        } else {
  ...
  const handleCharacterSelect = async (character: api.AICharacter) => {
    try {
      ...
      const convs = await api.getConversations();
      const filteredConvs = convs.filter(c => c.character_id === character.id);
      setConversations(filteredConvs);
      setActiveConversationId(newConv.id);
      setMessages([]);
